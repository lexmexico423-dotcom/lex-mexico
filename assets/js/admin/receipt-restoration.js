...
async function _cargarCfaiCreds(){
  try {
    const lsAcc = localStorage.getItem('lex-cfai-account') || '';
    const lsTok = localStorage.getItem('lex-cfai-token') || '';
    if(lsAcc) window._cfaiAccountCached = lsAcc;
    if(lsTok.length > 10) window._cfaiTokenCached = lsTok;
    if(!window.SB) return;
    const { data: sessionData } = await window.SB.auth.getSession();
    if(!sessionData?.session) return;
    const {data, error} = await window.SB.from('configuracion').select('id,valor').in('id',['cfai_account_id','cfai_api_token']);
    if(!error && Array.isArray(data)){
      data.forEach(row => {
        if(row.id === 'cfai_account_id' && row.valor){ window._cfaiAccountCached = row.valor.trim(); try{ localStorage.setItem('lex-cfai-account', row.valor.trim()); }catch(_){} }
        if(row.id === 'cfai_api_token' && row.valor && row.valor.length > 10){ window._cfaiTokenCached = row.valor.trim(); try{ localStorage.setItem('lex-cfai-token', row.valor.trim()); }catch(_){} }
      });
    }
  } catch(e){ console.warn('[CloudflareAI] Error cargando credenciales:', e.message); }
}
function _cfaiGetAccountId(){
  return window._cfaiAccountCached || localStorage.getItem('lex-cfai-account') || (document.getElementById('cfg-cfai-account')?.value||'').trim() || '';
}
function _cfaiGetToken(){
  return window._cfaiTokenCached || localStorage.getItem('lex-cfai-token') || (document.getElementById('cfg-cfai-token')?.value||'').trim() || '';
}
function _cfaiSaveCreds(accountId, token){
  accountId = (accountId||'').trim(); token = (token||'').trim();
  if(!accountId || !token) return;
  try{ localStorage.setItem('lex-cfai-account', accountId); localStorage.setItem('lex-cfai-token', token); }catch(e){ registrarError('localStorage.setItem', e); }
  window._cfaiAccountCached = accountId; window._cfaiTokenCached = token;
  if(window.SB){
    window.SB.from('configuracion').upsert([
      {id:'cfai_account_id', valor: accountId, updated_at: new Date().toISOString()},
      {id:'cfai_api_token',  valor: token,     updated_at: new Date().toISOString()}
    ]).then(()=>{ console.log('[CloudflareAI] ✅ Credenciales guardadas en Supabase'); if(typeof toast==='function') toast('☁️ Cloudflare Workers AI configurado','ok'); })
      .catch(e=> console.warn('[CloudflareAI] Error guardando en Supabase:', e));
  }
}
...
function ocrLimpiarTodo(){
  ...
  ['lex-gemini-key','lex-ocr-drive-token','lex-ocr-folder','lex-ocr-prof','lex-ocr-extra']
...
function ocrCargarKeyEnCfg(){
  ...
  if(geminiInp) geminiInp.value = (typeof ocrModGetKey==='function' ? ocrModGetKey() : '') || window._geminiKeyCached || localStorage.getItem('lex-gemini-key') || '';
...
function groqGuardarKey(){
  ...
  _groqSaveKey(key);
  if(st){ st.textContent = '✅ Key guardada correctamente'; st.style.color = 'var(--verde)'; }
...
function ocrGetKey(){
  return localStorage.getItem('lex-gemini-key') || '';
}
...
function ocrSaveKey(k){
  ...
  try{ localStorage.setItem('lex-gemini-key', keyTrim); } catch(e){ registrarError('localStorage.setItem', e); }
  // 2. Actualizar caché global inmediatamente
  window._geminiKeyCached = keyTrim;
  // 3. Persistir en Supabase
  if(window.SB && keyTrim.length > 10){
    window.SB.from('configuracion').upsert({id:'gemini_api_key', valor: keyTrim, updated_at: new Date().toISOString()})
      .then(()=>{ console.log('[Gemini] Key guardada en Supabase'); if(typeof toast==='function') toast('🔑 Gemini API Key guardada','ok'); })
      .catch(e=>console.warn('[Gemini] Error guardando key en Supabase:', e));
  }
}
...
function _intentarCargarKey(intento){
  ...
  const fromLS = localStorage.getItem('lex-gemini-key')||'';
  if(fromLS && fromLS.length > 10){ window._geminiKeyCached = fromLS; return; }
}
...
function geminiGuardarKey(){
  ...
  try{ localStorage.setItem('lex-gemini-key', k); }catch(e){ registrarError('localStorage.setItem', e); }
  if(window.SB && k.length > 10){
    window.SB.from('configuracion')
      .upsert({id:'gemini_api_key', valor: k, updated_at: new Date().toISOString()})
      .then(()=>console.log('[Gemini] ✅ Key guardada en Supabase'))
      .catch(e=>console.warn('[Gemini] Error guardando key:', e));
  }
  if(st){ st.textContent='✅ Key guardada'; st.style.color='var(--verde)'; }
  if(typeof toast==='function') toast('🔑 Gemini API Key guardada','ok');
}
async function geminiTestKey(){
  const inp = document.getElementById('cfg-gemini-key');
  const st  = document.getElementById('cfg-gemini-st');
  const k   = (inp?.value || '').trim() || window._geminiKeyCached || localStorage.getItem('lex-gemini-key') || '';
  if(!k){ if(st){ st.textContent='⚠ Ingresa una API Key'; st.style.color='var(--rojo)'; } return; }
  if(st){ st.textContent='🔄 Probando...'; st.style.color='var(--muted)'; }
  try{
...
// Reactivado a solicitud del despacho. La key ya estaba guardada en
// Supabase/localStorage desde la integración anterior (ocrModGetKey /
// gemini_api_key) — solo faltaba esta función tras haberla retirado en
// favor de Groq/Cloudflare. IMPORTANTE: usar solo con key de nivel de PAGO
